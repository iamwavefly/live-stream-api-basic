            ...
            if (!functions.empty(userExists)) {
                try {
                    ...
                    if (cache.has(cache_key)) {
                        ...
                        response.status(200).json({ "status": 200, "feedback": `User account details has been fetched successfully.`, "data": payload });
                        return true;
                    }
                    ...
                    response.status(200).json({ "status": 200, "feedback": "Workspace feedbacks has been fetched successfully.", "data": payload });
                
                } catch (e) {
                    response.status(400).json({ "status": 400, "feedback": e.feedback, "data": payload });
                }

            } else {
                response.status(400).json({ "status": 400, "feedback": "User account access authentication credentials failed, check and retry.", "data": payload });
            }

        } else {
            response.status(400).json({ "status": 400, "feedback": "Incomplete or missing requests parameter(s)", "data": null });
        }
