...
module.exports = function (app) {
    ...
    app.get(`/${endpoint_category}/get_templates`, async (request, response) => {
        ...
        let templateExists = await TEMPLATE.find()
        ...
        if (!functions.empty(userExists)) {
            try {
                ...
                payload["is_blocked"] = functions.stringToBoolean(userExists.is_blocked)
                payload["is_registered"] = functions.stringToBoolean(userExists.is_registered)
                payload["templates"] = messageExists,
                cache.set(cache_key, messageExists);
                response.status(200).json({ "status": 200, "message": "Templates has been fetched successfully.", "data": payload });
