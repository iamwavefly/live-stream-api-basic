        ...
        try {  
            ...
            if (request.query.token && request.query.workspace_id) {
                ...
                if (!functions.empty(userExists)) {
                    ...
                    // files.readwrite
                    let redirect_url = `https://login.live.com/oauth20_authorize.srf?client_id=${process.env.ONEDRIVE_CLIENT_ID}&scope=files.readwrite offline_access&response_type=code&redirect_uri=${REDIRECT_URL}&state=${`${request.query.token}_SEPARATOR_${request.query.workspace_id}`}`;
