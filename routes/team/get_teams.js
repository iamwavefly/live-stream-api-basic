...
module.exports = function (app) {
    ...
    app.get(`${endpoint_category}/get_teams`, async (request, response) => {
        ...
        */
        ...
        if (request.query.token && request.query.workspace_id) {
            ...
            let teamExists = await TEAM.find({ token: request.query.token, workspace_id: request.query.workspace_id})
            ...
            if (!functions.empty(userExists)) {
                try {
                    ...
                    cache.set(cache_key, teamExists);
                    response.status(200).json({ "status": 200, "message": "Workspace teams has been fetched successfully.", "data": payload });
