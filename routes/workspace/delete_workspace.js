        ...
        if (request.body.token && request.body.workspace_id) {
            ...
            if (!functions.empty(workspaceExists)) {
                try {
                    ...
                    await WORKSPACE.deleteMany({token: request.body.token, workspace_id: request.body.workspace_id });
                    ...
                    response.status(200).json({ "status": 200, "message": "User workspaces has been deleted successfully.", "data": payload });
