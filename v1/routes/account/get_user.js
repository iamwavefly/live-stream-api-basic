...
module.exports = function (app) {
    ...
    app.get(`${endpoint_category}/get_user`, async function (request, response) {
            ...
            if (!functions.empty(userExists)) {
                try {
                    ...
                    if (difference > process.env.TOKEN_EXPIRY_MINUTES) {
                        ...
                        payload["is_blocked"] = functions.stringToBoolean(userExists.is_blocked)
                        payload["is_registered"] = functions.stringToBoolean(userExists.is_registered)
                        response.status(400).json({ "status": 400, "message": "This user authentication token has expired, login agin retry.", "data": payload });
                        return true;
                    }
                    ...
                    response.status(400).json({ "status": 400, "message": e['name'] == 'SequelizeValidationError' ? e.errors[0].message : e.message, "data": payload });
                }
