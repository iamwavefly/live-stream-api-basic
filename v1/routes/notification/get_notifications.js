...
module.exports = function (app) {
    ...
    app.get(`${endpoint_category}/get_notifications`, async (request, response) => {
        ...
        */
        ...
        if (request.query.token && request.query.workspace_id) {
            ...
            let notificationExists = await NOTIFICATION.find({ token: request.query.token, workspace_id: request.query.workspace_id})
            ...
            if (!functions.empty(userExists)) {
                try {
                    ...
                    cache.set(cache_key, notificationExists);
                    response.status(200).json({ "status": 200, "message": "Workspace notifications has been fetched successfully.", "data": payload });
